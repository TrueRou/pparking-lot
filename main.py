...
async def has_data(source):
    async with db_session_bancho() as session:
        result = await session.execute(
            text("select id from scores_analysis where source=:source limit 1").bindparams(source=source))
        return result.first() is not None


async def delete_all_data():
    async with db_session_bancho() as session:
        await session.execute(text("delete from scores_analysis"))
        cache.cache_dict = {}
...
if __name__ == '__main__':
    uvicorn.run("main:app", host="127.0.0.1", port=8727)
